...
def train_epoch(model, optimizer, train_loader, criteria, loss_train, acc_train, topk_acc_train, list_k, n_train, use_gpu):
    ...
    for batch_idx, (batch_x_train, batch_y_train) in enumerate(tqdm(train_loader, desc='train', position=0)):
        batch_x_train = torch.Tensor(batch_x_train) # GREENSTAND - Convert to correct format
...
def val_epoch(model, val_loader, criteria, loss_val, acc_val, topk_acc_val, avgk_acc_val,
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_x_val, batch_y_val) in enumerate(tqdm(val_loader, desc='val', position=0)):
            batch_x_val = torch.Tensor(batch_x_val) # GREENSTAND - Convert to correct format
            if use_gpu:
...
def test_epoch(model, test_loader, criteria, list_k, lmbda, use_gpu, dataset_attributes):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_x_test, batch_y_test) in enumerate(tqdm(test_loader, desc='test', position=0)):
            batch_x_test = torch.Tensor(batch_x_test) # GREENSTAND - Convert to correct format
            if use_gpu:
            ...
            confuse = add_to_confusion_matrix(pred=torch.argmax(batch_output_test, dim=-1), labels=batch_y_test, confusion_matrix=confuse)  # GREENSTAND
    ...
    confuse = finish_confusion_matrix(confuse, dataset_attributes)

    return loss_epoch_test, acc_epoch_test, topk_acc_epoch_test, avgk_acc_epoch_test, class_acc_dict, confuse

def generate_confusion_matrix(classes): # GREENSTAND
    confusion_matrix = {}
    # Create classes
    for c in classes:
        mat = {'tp' : 0, 'fp' : 0, 'tn' : 0, 'fn': 0}
        confusion_matrix[c] = mat
    confuse = pd.DataFrame(confusion_matrix)
    return confuse

def add_to_confusion_matrix(pred, labels, confusion_matrix): # GREENSTAND 
    pred = pred.cpu().data.numpy()
    labels = labels.cpu().data.numpy()
    
    for c in confusion_matrix.columns: # each class
        # TP
        true_labels = np.where(labels == c)[0]
        truth = labels[true_labels]
        preds = pred[true_labels]
        tp = np.where(preds == c)[0].shape[0]
        
        # FN
        fn = preds.shape[0] - tp
        
        # FP
        true_preds = np.where(preds == c)[0]
        truth = labels[true_preds]
        preds = pred[true_preds]
        diff = truth - preds
        fp = np.where(diff != 0)[0].shape[0]
        
        # TN
        tn = pred.shape[0] - tp - fn - fp
        
        # Set
        confusion_matrix.loc['tp', c] = tp + confusion_matrix.loc['tp', c]
        confusion_matrix.loc['fn', c] = fn + confusion_matrix.loc['fn', c]
        confusion_matrix.loc['fp', c] = fp + confusion_matrix.loc['fp', c]
        confusion_matrix.loc['tn', c] = tn + confusion_matrix.loc['tn', c]
        
    return confusion_matrix

def finish_confusion_matrix(confuse, dataset_attributes): # GREENSTAND 
    # Transpose
    classes = confuse.columns
    confuse = confuse.transpose()
    
    # Add total num instance
    confuse['num_instances'] = 0
    for c in classes:
        num = dataset_attributes['class2num_instances']['test'][c]
        confuse.loc[c, 'num_instances'] = num
    
    # Replace class numbers with class names
    idx_to_class = {value:key for key, value in dataset_attributes['class_to_idx'].items()}
    confuse = confuse.rename(idx_to_class)
    
    # Accuracy, Precision, Recall
    confuse['accuracy'] = confuse['tp'] / confuse['num_instances']
    confuse['precision'] = (confuse['tp']) / (confuse['tp'] + confuse['fp'])
    confuse['recall'] = (confuse['tp']) / (confuse['tp'] + confuse['fn'])
